...
@section('content')
    ...
    <div class="row">
        <form action=" {{ route('saveEdit', $transaction->transaction_id) }}" method="POST">
            @csrf
            @method('put')
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-12">
                        <div class="mb-3">
                            ...
                            <select id="transaction_customer_id" name="transaction_customer_id" class="form-select">
                                @foreach ($catalogs['customers'] as $customer)
                                    <option value="{{ $customer->customer_id }}"
                                        {{ old('transaction_customer_id', $transaction->transaction_customer_id) == $customer->customer_id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-lg-6 col-md-12">
                        <div class="mb-3">
                            ...
                            <select id="transaction_employe_id" name="transaction_employe_id" class="form-select">
                                @foreach ($catalogs['employees'] as $employe)
                                    <option value="{{ $employe->employe_id }}"
                                        {{ old('transaction_employe_id', $transaction->transaction_employe_id) == $employe->employe_id ? 'selected' : '' }}>
                            ...
                            <select id="transaction_type_movement_id" name="transaction_type_movement_id"
                                ...
                                @foreach ($catalogs['typesMovements'] as $mov)
                                    <option value="{{ $mov->type_movement_id }}"
                                        {{ old('transaction_type_movement_id', $transaction->transaction_type_movement_id) == $mov->type_movement_id ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12">
                        <div class="mb-3">
                            ...
                            <select id="transaction_product_id" name="transaction_product_id" class="form-select">
                                @foreach ($catalogs['products'] as $product)
                                    <option value="{{ $product->product_id }}"
                                        {{ old('transaction_product_id', $transaction->transaction_product_id) == $product->product_id ? 'selected' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-12">
                        <div class="mb-3">
                            ...
                            <input id="transaction_price" name="transaction_price" type="number" class="form-control"
                                value="{{ old('transaction_price', $transaction->transaction_price) }}" />
                        </div>
                    </div>
                </div>
